...
def main(config):
    ...
    with torch.no_grad():
        for idx, (data, gt, mask, target) in enumerate(tqdm(data_loader)):
            ...
            # save masks
            save_path = os.path.join('saved', dataset, 'masks', str(idx) + '.png')
            scipy.misc.imsave(save_path, mask[0].cpu().numpy())
            # save gt
            ...
            save_path = os.path.join('saved', dataset, 'cams_array', str(idx) + '.npy')
